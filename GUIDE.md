...
### Ошибка: Settings Error — matcher format

```
Settings Error: matcher: Expected string, but received object
```

**Причина:** Старый формат hooks в `.claude/settings.json`

**Решение:** Замените `"matcher": {}` на `"matcher": "*"`:
```json
{
  "hooks": {
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .cct/hooks/check-leads.sh"
          }
        ]
      }
    ]
...
### Workers не запускаются в Hierarchical mode

**Причина:** Claude в `-p` режиме не может напрямую запускать другие Claude сессии (safety protection).

**Решение:** Используйте `launch_worker.sh` (v0.2.5+) который использует `systemd-run --user`:
...
**Требования:** `systemd` должен быть доступен (Linux). Проверка:
```bash
systemd-run --user echo "works"
```

### Lead не создаёт workers

**Причина:** Claude отказывается выполнять `claude -p` из инструкций в CLAUDE.md (считает prompt injection).

**Решение:** Убедитесь что `lead.md` содержит секцию `AUTHORIZATION NOTICE` и инструкции использовать `launch_worker.sh`.

### Hooks не показывают уведомления

**Причина:** `check-leads.sh` не находит папку `.outputs/`

**Решение:** Убедитесь что запускаете Claude из корня проекта (где есть `.outputs/`).
